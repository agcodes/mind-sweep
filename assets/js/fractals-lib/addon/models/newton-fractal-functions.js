...
export default class NewtonFractalFunctions {
	getDerivativeFunction(type) {
		const functions = {
			// z^n-c => n.z^(n-1)
			'f1': (z, n) => z.pow(n - 1).mulValues(n, 0, 0),
			// z+z^n+c => 1 + n.z^n-1
			'f2': (z, n) => (new CComplex(n, 0).mul(z.pow(n - 1))).addValues(1, 0, 0),
			// z^n+c => n.z^n-1
			// z^2+c => 2.z
			'f4': (z, n) => z.pow(n - 1).mulValues(n, 0, 0),
			// f(z^n)-c^n => 2.z
			'f7': (z, n) => z.pow(n - 1).mulValues(n, 0, 0),
			'f8': (z, n) => new CComplex(n, 0).mul(z.pow(n - 1)),
			'f10': (z, n) => new CComplex(n, 0).mul(z.pow(n - 1)).addValues(-1, 0, 0),
			'f31': (z, n) => new CComplex(1, 0).sub(z.pow(n - 1).mulValues(n, 0, 0))
		};
	...
	getFunction(name) {
		const functions = {
			"multiple": (z, n, f, c, c2, args) => z.mul(new CComplex(args[0], args[1])).getC(f),
			// f(z^n)-c
			...
			// f(z)^n-z+c
			'f10': (z, n, f, c) => z.getC(f).pow(n).sub(z).addIn(c),
			// f(z.(c+z^n+z)+c2-z)
