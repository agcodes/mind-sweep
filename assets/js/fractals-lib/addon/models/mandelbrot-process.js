...
export default class MandelbrotProcess extends FractalProcess {
    ...
    getPts(nb) {
        ...
        for (let index = 0; index < nb; index++) {
            ...
            if (pt) {
                if (this.params.animation.z0 !== 0) {
                    pts1.push([ptCanvas[0], ptCanvas[1], this.params.z0 * 12, pt[3], pt[4]]);
    ...
    getPt(ptCanvas) {
        if (this.gridAdapter.isInCoordLimits(ptCanvas)) {
            ...
            this.nbPts++;
            return this.fractal.getPt(pt[0], pt[1], this.params.z0, this.params.d3, ptCanvas, this.recycle);
        }
        return null;
    }
}