...
export default class FractalParameters {
	...
	setData(data) {
		...
		if (data) {
			...
			if (this.relativeC2) {
				...
				this.c2 = (typeof data.c2 === "undefined") ? new CComplex(0, 0, 0) : new CComplex(data.c2.a, data.c2.b);
			}

			this.zFromOrigin = (typeof data.zFromOrigin === 'boolean') ? data.zFromOrigin : false;
			if (this.zFromOrigin) {
				this.zOrigin = new CComplex(0, 0, 0);
			}
			else {
				this.zOrigin = (typeof data.zOrigin === "undefined") ? null : new CComplex(data.zOrigin.a, data.zOrigin.b);
			}

			this.multiple = (typeof data.multiple === "undefined") ? null : new CComplex(data.multiple.r, data.multiple.i);
			this.d3 = (typeof data.d3 === 'number') ? data.d3 : 0;
			this.z0 = (typeof data.z0 === 'number') ? data.z0 : 0;
	...
	setInnerColor(c2, colorsService) {
		if (this.colorsService) {
			...
			if (this.innerMethod == 0) {
				if ([1, 2, 3, 4, 5, 9, 10].includes(this.method)) {
	...
	setVarParameters(colorsService) {
		if (colorsService && this.animation && this.animation.mode && this.animation.mode > 0) {
			if (this.animation.mode === 2) {
				//this.out = false;
				//this.inner = true;
				//this.innerColor[2] += this.animation.z0 * 80;

				if (this.animation.innerColor) {
					this.innerColor[0] = Math.round(this.innerColor_o[0] + Math.sin(this.z0) * this.animation.innerColor[4]);
					this.innerColor[1] = Math.round(this.innerColor_o[1] + Math.sin(this.z0) * this.animation.innerColor[5]);
					this.innerColor[2] = Math.round(this.innerColor_o[2] + Math.sin(this.z0) * this.animation.innerColor[6]);
	...
	applyAnimationVariation(colorsService) {
		if (this.animation) {
			...
			if (typeof this.animation.z0 === "number") {
				this.z0 += this.animation.z0;
				this.z0 = this.roundFloat(this.z0);
			}
			...
			if (this.animation.multiple && this.multiple != null && typeof this.animation.base === "object") {
