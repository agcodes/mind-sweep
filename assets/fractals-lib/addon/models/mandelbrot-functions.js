...
export default class MandelbrotFunctions {
	getDerivativeFunction(type) {
		const functions = {
			// z^n+c => n.z^(n-1)
			'f1': (z, n) => new Complex(n, 0).mul(z.pow(n - 1))
		};
	...
	getFunction(name) {
		const functions = {
			"multiple": (z, n, c, f, args) => z.mul(new Complex(args[0], args[1])).getC(f),
			...
			"f1_rand": (z, n, c, f, args) => {
				if (Math.random() < 0.5) {
					return z.pow(n).addIn(c).getC(f);
				}
				else {
					return z.pow(n).addIn(new Complex(args.r, args.i, 0)).getC(f);
				}
			},
			"f1_rand_b": (z, n, c, f, args) => {
				if (Math.random() < 0.5) {
					return new Complex(
						z.r * z.r - z.i * z.i + args.r,
						z.r * z.i + args.i
					);
				}
				else {
			...
			"f3_alt_rot": (z, n, c, f, args, i) => {
				if (i % 2 != 0) {
					if (typeof args.rot == "undefined") {
						return null;
					}
					else {
						return z.abs().pow(n).rot(args.rot).addIn(c).getC(f);
					}
				}
				return z.abs().pow(n).addIn(c).getC(f);
			},
			...
			// -0.1
			// f(|z|^n+c+c_args)
			"f3-3": (z, n, c, f, args) => z.abs().pow(n).add(c).add(new Complex(args[0], args[1])).getC(f),
			// f(c.z^n)
			...
			// f(z^n)+c
			"f14": (z, n, c, f, args) => z.pow(n).getC(f).addIn(c),
			// |z|n-c
			...
			"f94": (z, n, c, f, args) => c.add(z.pow(n).sub(z.mulReal(-1).pow(n + (typeof args[0] !== "undefined" ? args[0] : 1)))).getC(f),
			...
			"f118": (z, n, c, f, args) => z.pow(n).add(c).add(new Complex(args[0] ? args[0] : 0, args[1] ? args[1] : 0)).getC(f),
			"f119": (z, n, c, f, args) => z.pow(n).add(c).mul(new Complex(args[0] ? args[0] : 0, args[1] ? args[1] : 0)).getC(f),
			"f120": (z, n, c, f, args) => c.pow(n).add(z).getC(f),
			"f121": (z, n, c, f, args) => c.mul(z.pow(n).mul(new Complex(args[0] ? args[0] : 0, args[1] ? args[1] : 0)).getC(f)),
			...
			"f127": (z, n, c, f, args) => z.abs().pow(n).div(c).abs().add(c.powComplex(c)).abs().getC(f),
			"0": (z, n, c, f, args) => z,
	...
	getFunctionDefinition(name, f, n, args, c) {
		const functions = {
			...
			"j_o": () => 'z^n+c',
			"f1": () => 'f(z^n+c)',
			"f1_alt_rot": () => 'f(z^n+c) || f(rot(z^n)+c)',
			"f2": () => 'f(z^n-c)',
			// like burning ship
			"f3": () => 'f(|z|^n+c)',
			// like burning ship
			...
			"f94": (n, args) =>
				"",
			"f95": (n, args) => (args[0] === 1) ? `f(z^n-(z^${n + (typeof args[1] !== "undefined" ? args[1] : -1.0001)})+c)` :
				`f(z^n-((${typeof args[0] !== "undefined" ? args[0] : -1},0).z^${n + (typeof args[1] !== "undefined" ? args[1] : -1.0001)})+c)`,
